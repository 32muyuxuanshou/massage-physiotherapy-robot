...
start=time.time()
assert json.loads((O/'report.json').read_text())['status']=='COMPLETED'
checkpoint=torch.load(O/'heads_epoch2.pt',map_location='cuda',weights_only=False)
assert checkpoint['epoch']==2
assert checkpoint['manifest_sha256']==hashlib.sha256((O/'samples.json').read_bytes()).hexdigest()
for k,h in heads.items():h.load_state_dict(checkpoint['heads'][k])
opt=torch.optim.AdamW(params,lr=protocol['lr'],weight_decay=protocol['weight_decay'])
opt.load_state_dict(checkpoint['optimizer'])
baseline=json.loads((O/'baseline.json').read_text())['image_macro']
history=json.loads((O/'history.json').read_text());assert history[-1]['epoch']==2
updates=history[-1]['updates'];rng=np.random.default_rng(protocol['seed'])
for _ in range(2):rng.permutation(len(train))
protocol['epochs']=10
protocol['continuation']='Resume epoch2 heads and AdamW; reconstruct sampler state by replaying two seeded permutations; original two-epoch protocol retained'
(O/'continuation_protocol.json').write_text(json.dumps(protocol,indent=2))
print('RESUMED epoch2; extending total to 10 epochs',flush=True)
for epoch in range(2,protocol['epochs']):
...
(O/'continuation_report.json').write_text(json.dumps(report,indent=2)); print(json.dumps(report),flush=True)
