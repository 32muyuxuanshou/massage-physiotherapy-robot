...
def loss_for(out,r):
 ...
 fitted=F.smooth_l1_loss(p2/size,t2/size)+F.smooth_l1_loss(p3-p3[:,pelvis].mean(1,keepdim=True),t3-t3[:,pelvis].mean(1,keepdim=True))
 if r['split']=='validation' or not r['manual_eligible']:return fitted
 manual=p2.new_tensor(r['coco_annotation']['keypoints']).reshape(17,3)[5:17]
 visible=manual[:,2]==2
 extra=F.smooth_l1_loss(p2[0,mapping][visible]/size,manual[visible,:2]/size) if visible.any() else p2.sum()*0
 return fitted+protocol['manual_weight']*extra
def evaluate(tag):
 metrics=[]
 with torch.no_grad():
