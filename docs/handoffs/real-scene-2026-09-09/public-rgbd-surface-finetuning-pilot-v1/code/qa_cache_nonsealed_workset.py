...
def assert_nonsealed_contract(plan: dict, workset_root: Path) -> None:
    if plan.get("sealed_pixels_selected") is not False:
        raise RuntimeError("plan does not explicitly exclude SEALED pixels")
    sealed = set(plan["sealed_subjects_excluded"])
...
def main() -> None:
    ...
    if extraction.get("sealed_pixels_prepared") is not False:
    ...
    write_json(args.output_root / "NONSEALED_RGBD_REGISTRATION_QA_V1.json", report)
