"""QA/cache the authorized SEALED-only HuMMan workset using the prior geometry module."""
...
def assert_sealed_contract(plan: dict, workset_root: Path) -> None:
    expected_root = Path("/raid5/xuhd/public_rgbd_surface_finetuning_pilot_v1/workset_sealed_v1")
    if workset_root.resolve() != expected_root or plan.get("sealed_pixels_selected") is not True:
        raise RuntimeError("requires authorized SEALED plan and isolated fixed server workset")
    rows = plan["observations"]
    subjects = {row["subject"] for row in rows}
    if subjects != set(plan["authorized_sealed_subjects"]) or len(subjects) != 5:
        raise RuntimeError("not the exact five authorized SEALED subjects")
    if any(row["split"] != "SEALED" for row in rows) or subjects & set(plan["nonsealed_subjects_excluded"]):
        raise RuntimeError("nonsealed subject in SEALED-only plan")
    sequences = {row["sequence"] for row in rows}
    present = {p.name for p in workset_root.iterdir() if p.is_dir()}
    if present != sequences:
        raise RuntimeError("workset directories differ from fixed SEALED sequence set")
...
def main() -> None:
    ...
    if extraction.get("status") != "DECODE_FRAME_ID_QA_PASS_GEOMETRY_NOT_YET_VALIDATED":
        raise RuntimeError(f"selective extraction is not complete: {extraction.get('status')}")
    if extraction.get("sealed_pixels_prepared") is not True:
        raise RuntimeError("extraction report does not confirm authorized SEALED extraction")
    assert_sealed_contract(plan, args.workset_root)
    if args.output_root.resolve() != Path("/raid5/xuhd/public_rgbd_surface_finetuning_pilot_v1/qa_cache_sealed_v1"):
        raise RuntimeError("SEALED cache must use isolated fixed output root")
    ...
    report = {"status": "AUTO_QA_COMPLETE_VISUAL_REVIEW_PENDING", "task": "PUBLIC_RGBD_SURFACE_FINETUNING_PILOT_V1",
              "scope": "AUTHORIZED_SEALED_EVALUATION_ONLY", "sealed_pixels_read": True, "checkpoint_selection": "E1 already selected from VAL; no reselection permitted",
              "source_plan": str(args.plan.resolve()), "source_plan_sha256": sha256(args.plan),
              "source_split_sha256_from_plan": plan["source_split_sha256"],
    ...
    write_json(args.output_root / "SEALED_RGBD_REGISTRATION_QA_V1.json", report)
