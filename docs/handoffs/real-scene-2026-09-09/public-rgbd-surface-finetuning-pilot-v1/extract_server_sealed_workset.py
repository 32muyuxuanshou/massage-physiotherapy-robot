"""Authorized first SEALED opening: five fixed subjects, evaluation-only isolated workset."""
import argparse
import gc
import hashlib
import json
...
WORKSET = Path('/raid5/xuhd/public_rgbd_surface_finetuning_pilot_v1/workset_sealed_v1')
CAMERAS = ('kinect_008', 'kinect_009')
PLAN_NAME = 'SERVER_SEALED_SELECTIVE_EXTRACTION_PLAN_V1.json'
...
def make_plan(root):
    ...
    plan = {'status':'READY_FOR_AUTHORIZED_SEALED_SERVER_SELECTIVE_EXTRACTION',
        ...
        'counts_by_split':{k:sum(r['split']==k for r in observations) for k in ('SEALED',)},
        'sealed_pixels_selected':True, 'nonsealed_subjects_excluded':sum([v for k,v in split['candidate_subjects'].items() if k!='SEALED'], []), 'authorized_sealed_subjects':split['candidate_subjects']['SEALED'], 'authorization': 'First SEALED opening after E10 completion and E1 selection using VAL; evaluation only, no training or checkpoint reselection',
        ...
        'solid_archive_warning':'Selected members alone are written, but solid archive decoding can still read/decompress earlier unrelated streams internally. Only the five authorized SEALED subjects are materialized. Nonsealed members are not written.',
...
def execute(root):
    ...
    assert plan['sealed_pixels_selected'] is True
    assert set(r['subject'] for r in plan['observations']) == set(plan['authorized_sealed_subjects'])
    assert len(plan['authorized_sealed_subjects']) == 5
    assert all(r['split'] in ('SEALED',) for r in plan['observations'])
    assert not (set(r['subject'] for r in plan['observations']) & set(plan['nonsealed_subjects_excluded']))
    ...
    report = {'status':'RUNNING','free_bytes_before':free,'plan_sha256':sha(root/PLAN_NAME),
              'archives':[],'views':[],'sealed_pixels_prepared':True}
    report_path = root/'SERVER_SEALED_WORKSET_EXTRACTION_QA_V1.json'
