...
TYPES={'DISTANCE_MM','RATIO_0_1','SIGNED_RATIO_M1_1','COUNT','BOOLEAN','VECTOR_MM','HASH_IDENTITY'}
def icc_a1(matrix):
...
def classify(matrix,contract,outcomes=None):
 ...
 if kind=='VECTOR_MM':return classify_vector(matrix,contract)
 if kind=='BOOLEAN':
...
def classify_vector(values,contract):
 x=np.asarray(values,float)
 if x.ndim not in (3,4) or x.shape[-1]!=3:raise ValueError('VECTOR_MM_EXPECTS_FRAME_RERUN_OPTIONAL_ITERATION_XYZ')
 if x.ndim==3:x=x[:,:,None,:]
 scalar={**contract,'type':'DISTANCE_MM'};details={};rank={'STABLE':0,'CAUTION':1,'UNSTABLE':2};worst='STABLE'
 for iteration in range(x.shape[2]):
  vectors=x[:,:,iteration,:]
  for index,name in enumerate(('x','y','z')):
   result=classify(vectors[:,:,index],scalar);details[f'iter{iteration+1}_{name}']=result
   if rank[result['status']]>rank[worst]:worst=result['status']
  result=classify(np.linalg.norm(vectors,axis=-1),scalar);details[f'iter{iteration+1}_norm']=result
  if rank[result['status']]>rank[worst]:worst=result['status']
 return {'aggregation_rule':'WORST_COMPONENT','components':['x','y','z','norm'],'per_iteration':values is not None and np.asarray(values).ndim==4,'details':details,'status':worst}
