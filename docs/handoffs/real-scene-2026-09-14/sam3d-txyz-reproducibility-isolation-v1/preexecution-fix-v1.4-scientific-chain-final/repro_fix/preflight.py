...
def check(root,run_a_manifest):
 ...
 master=json.loads((root/'MASTER_ORCHESTRATION_SPEC_V1_4.json').read_text());assert master['fail_closed'] and not master['formal_execution_authorized_in_this_delivery'];assert master['ordered_stages'][:5]==['reviewed_delivery_integrity_pre','runtime_assets','run_a_actual_assets','input_snapshot','pointcloud_manifest_freeze'];assert 'sam_cohort_txyz' in master['ordered_stages'];assert master['ordered_stages'][-1]=='post_execution_integrity_including_reviewed_delivery';assert REVIEWED_TAG=='sam3d-txyz-repro-v1.4.1'
