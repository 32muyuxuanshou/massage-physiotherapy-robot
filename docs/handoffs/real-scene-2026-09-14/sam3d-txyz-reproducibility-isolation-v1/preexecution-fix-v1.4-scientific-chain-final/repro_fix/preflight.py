...
def check(root,run_a_manifest):
 ...
 master=json.loads((root/'MASTER_ORCHESTRATION_SPEC_V1_4.json').read_text());assert master['fail_closed'] and not master['formal_execution_authorized_in_this_delivery'];assert master['ordered_stages'][:4]==['runtime_assets','run_a_actual_assets','input_snapshot','pointcloud_manifest_freeze'];assert 'sam_cohort_txyz' in master['ordered_stages'];assert master['ordered_stages'][-1]=='post_execution_integrity'
 ...
 return {'status':'PASS_SCIENTIFIC_CHAIN_PREFLIGHT_V1_4','sentinel_unique':'7/7','run_a_frames':freeze['expected_frame_count'],'run_a_actual_hash_gate':'FORMAL_RUNTIME_REQUIRED_45_POINTS_PLUS_45_ANCHORS','pointcloud_manifest_source':'DERIVED_FROM_FORMAL_45','sam_to_txyz_chain':'RUN_B_TO_F_ANCHORS','feature_contract_coverage':f'{len(contract["features"])}/{len(contract["features"])}','master_fail_closed':True,'post_execution_integrity':True,'calibration_files_frozen':20,'formal_experiment_executed':False}
def main():
