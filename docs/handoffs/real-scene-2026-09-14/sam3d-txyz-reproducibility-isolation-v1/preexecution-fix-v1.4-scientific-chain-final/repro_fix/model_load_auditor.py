...
def load_and_audit(sam_repo,checkpoint,mhr,output,mode='CONTROLLED',seed=20260912):
 import torch
 seed_applied=False
 if mode=='CONTROLLED':apply_reproducibility_environment(seed);seed_applied=True
 sys.path.insert(0,str(sam_repo));from sam_3d_body import load_sam_3d_body
 model,_=load_sam_3d_body(str(checkpoint),device='cuda',mhr_path=str(mhr));raw=torch.load(checkpoint,map_location='cpu',weights_only=False);state=raw.get('state_dict',raw.get('model',raw));model_keys=set(model.state_dict());checkpoint_keys=set(state);missing=sorted(model_keys-checkpoint_keys);unexpected=sorted(checkpoint_keys-model_keys);result={**audit_model(model,missing,unexpected),'mode':mode,'seed':seed if seed_applied else None,'seed_applied':seed_applied};Path(output).write_text(json.dumps(result,indent=2)+'\n');return result
def run_fresh(a):
 if not a.authorized_formal:raise RuntimeError('FORMAL_RUN_REQUIRES_MASTER_ORCHESTRATOR_GO')
 require_master_authorization()
 rows=[]
 for i in range(a.runs):
  out=a.output.with_suffix(f'.run{i}.json');cmd=[sys.executable,'-m','repro_fix.model_load_auditor','--worker','--authorized-formal','--sam-repo',str(a.sam_repo),'--checkpoint',str(a.checkpoint),'--mhr',str(a.mhr),'--output',str(out),'--mode',a.mode];
  if a.mode=='CONTROLLED':cmd += ['--seed',str(a.seed)]
  subprocess.run(cmd,check=True);rows.append(json.loads(out.read_text()));out.unlink()
 stable=fingerprints_equal(rows);reported=sorted({k for row in rows for k in row['hard_block_keys']});result={'fresh_processes':a.runs,'fingerprints_equal':stable,'loader_reported_missing_trainable_keys':reported,'loader_missing_keys_retained_for_audit':True,'runs':rows,'status':'PASS_MODEL_LOAD_REPRODUCIBILITY' if stable else 'MODEL_LOAD_REPRODUCIBILITY_HARD_BLOCK'};a.output.write_text(json.dumps(result,indent=2)+'\n');return result
def main():
 p=argparse.ArgumentParser();p.add_argument('--worker',action='store_true');p.add_argument('--authorized-formal',action='store_true');p.add_argument('--runs',type=int,default=5);p.add_argument('--mode',choices=['CONTROLLED','UNSEEDED_DIAGNOSTIC'],required=True);p.add_argument('--seed',type=int,default=20260912);p.add_argument('--sam-repo',type=Path,required=True);p.add_argument('--checkpoint',type=Path,required=True);p.add_argument('--mhr',type=Path,required=True);p.add_argument('--output',type=Path,required=True);a=p.parse_args()
 if not a.authorized_formal:raise RuntimeError('FORMAL_RUN_REQUIRES_MASTER_ORCHESTRATOR_GO')
 require_master_authorization()
 if a.worker:load_and_audit(a.sam_repo,a.checkpoint,a.mhr,a.output,a.mode,a.seed)
 else:run_fresh(a)
