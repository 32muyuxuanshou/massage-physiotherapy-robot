...
class Tests(unittest.TestCase):
 ...
 def test_22_pointcloud_fresh_process(self):
  with tempfile.TemporaryDirectory() as t:
   t=Path(t);cv2.imwrite(str(t/'d.png'),np.array([[1000]],np.uint16));cv2.imwrite(str(t/'m.png'),np.array([[255]],np.uint8));np.save(t/'q.npy',np.array([[[0.,0.]]],np.float32));(t/'x.json').write_text('{}');row={'frame_id':'f','depth':str(t/'d.png'),'mask':str(t/'m.png'),'pointcloud_table':str(t/'q.npy'),'calibration':str(t/'x.json')};row.update({f'{key}_sha256':file_sha(row[key]) for key in ('depth','mask','pointcloud_table','calibration')});(t/'manifest.json').write_text(json.dumps({'rows':[row]}));r=coordinator(t/'manifest.json',t/'out.json',2);self.assertEqual(r['status'],'POINTCLOUD_REPRODUCIBILITY_PASS')
 ...
 def test_27_run_a_freeze(self):
  with tempfile.TemporaryDirectory() as t:
   t=Path(t);rows=[]
   for i in range(45):
    points=t/f'p{i}.npz';anchors=t/f'a{i}.npz';np.savez(points,points=np.zeros((1,3)));np.savez(anchors,anchors=np.zeros((1,3)));rows.append({'frame_id':str(i),'points_npz':str(points),'anchors_npz':str(anchors),'points_sha256':file_sha(points),'anchors_sha256':file_sha(anchors)})
   p=t/'m.json';p.write_text(json.dumps({'rows':rows}));f=build(p);self.assertEqual(verify(f,p)['status'],'PASS_RUN_A_ACTUAL_ASSET_FREEZE');self.assertEqual(f['expected_frame_count'],45)
 ...
 def test_29_dataset_mask_boundary_retained(self):self.assertIn('dataset-mask-assisted', (ROOT/'README_PREEXECUTION_REPRO_FIX_V1_4_SCIENTIFIC_CHAIN_FINAL.md').read_text())
 def test_30_back_fail_closed(self):self.assertIn('Back-region, DMD37 and medical acupoint conclusions remain outside',(ROOT/'README_PREEXECUTION_REPRO_FIX_V1_4_SCIENTIFIC_CHAIN_FINAL.md').read_text())
 ...
 def test_44_model_state_mutation(self):
  arr={'vertices':np.zeros((1,3)),'cam_t':np.zeros(3),'anchors':np.zeros((1,3))};a={'prepared_tensor_hash':'i','cam_int_hash':'k','model_state_before':{'model_state_sha256':'m1'},'model_state_after':{'model_state_sha256':'m1'},'arrays':arr};b={**a,'model_state_before':{'model_state_sha256':'m2'},'model_state_after':{'model_state_sha256':'m2'}};self.assertEqual(compare_order_runs({'A':{'f':a},'B':{'f':b}})['status'],'MODEL_STATE_MUTATION')
 ...
 def test_57_legacy_complete_run_a_analyzer_blocked(self):
  with self.assertRaisesRegex(RuntimeError,'HISTORICAL_RUN_A_COMPLETE_SCHEMA_FORBIDDEN'):analyze_sam('a',['b'])
