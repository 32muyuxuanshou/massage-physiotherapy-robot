...
def sam_record(fid,npz):
 z=np.load(npz);return {'frame_id':fid,'arrays_npz':str(npz),'pred_vertices':array_record(z['vertices']),'anchors':array_record(z['anchors']),'prepared_tensor':array_record(np.zeros(1)),'cam_int':array_record(np.eye(3)),'model_state_before_frame':{'model_state_sha256':'m'},'model_state_after_frame':{'model_state_sha256':'m'}}
...
class Integration(unittest.TestCase):
 def test_01_historical_boundary_is_anchor_only(self):
  b=json.loads((ROOT/'SAM_REPRODUCIBILITY_EVIDENCE_BOUNDARY_V1_3.json').read_text());self.assertEqual(b['historical_run_a']['permitted_comparison'],'anchors_only');self.assertIn('full_vertices',b['historical_run_a']['unavailable'])
 ...
 def test_07_master_stage_order(self):self.assertEqual(STAGES[0],'model_load_unseeded');self.assertEqual(STAGES[-1],'feature_stability')
 ...
 def test_10_asset_verifier_pass_and_mismatch(self):
  with tempfile.TemporaryDirectory() as t:
   t=Path(t);[(t/x).write_text(x) for x in ('ckpt','mhr','anchor','metric','manifest','txyz','cal')];repo=t/'repo';repo.mkdir();(repo/'a.py').write_text('x');helper=t/'helper';helper.mkdir();(helper/'behave_v2_io.py').write_text('h');paths={'official_checkpoint':t/'ckpt','mhr_model':t/'mhr','surface_anchors':t/'anchor','surface_metrics':t/'metric','sam3d_repo':repo,'v23_code':helper,'formal_manifest':t/'manifest','txyz_implementation':t/'txyz','calibration_root':t,'calibration_relative_files':['cal']};obs={'official_checkpoint_sha256':file_sha(t/'ckpt'),'mhr_model_sha256':file_sha(t/'mhr'),'surface_anchors_sha256':file_sha(t/'anchor'),'surface_metrics_sha256':file_sha(t/'metric'),'sam3d_source_tree_sha256':tree_sha(repo),'v23_helper_code_sha256':file_sha(helper/'behave_v2_io.py')};gate={'formal_manifest_sha256':file_sha(t/'manifest'),'txyz_implementation_sha256':file_sha(t/'txyz'),'camera_calibration_bundle_sha256':bundle_sha(t,['cal'])};freeze={'assets':obs,'formal_runtime_gate':gate};self.assertEqual(verify_assets(paths,freeze)['status'],'PASS_RUNTIME_ASSET_FREEZE');freeze['assets']['mhr_model_sha256']='bad';self.assertEqual(verify_assets(paths,freeze)['status'],'ASSET_FREEZE_MISMATCH')
 def test_11_input_snapshot_hashes_rgb_mask(self):
  with tempfile.TemporaryDirectory() as t:
   t=Path(t);f=t/'seq'/'frame';f.mkdir(parents=True);cv2.imwrite(str(f/'k0.color.jpg'),np.zeros((2,2,3),np.uint8));cv2.imwrite(str(f/'k0.person_mask.jpg'),np.zeros((2,2),np.uint8));m=t/'m.json';m.write_text(json.dumps({'rows':[{'frame_id':'f','sequence':'seq','frame':'frame'}]}));self.assertIn('raw_rgb_file_sha256',build_snapshot(m,t)['rows'][0])
 ...
 def test_15_unsaved_history_boundary_in_readme(self):self.assertIn('No code can recover data that was not saved',(ROOT/'README_PREEXECUTION_REPRO_FIX_V1_3_INTEGRATION_FINAL.md').read_text())
