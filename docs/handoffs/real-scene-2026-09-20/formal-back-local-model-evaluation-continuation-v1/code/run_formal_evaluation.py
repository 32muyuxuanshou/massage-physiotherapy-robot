...
def crop_boxes(mask):
    ...
    return {'FULL':b(-.05,-.05,1.05,1.05),'UPPER':b(.02,0,.98,.72),'LOCAL_TORSO':b(.20,.15,.80,.72)}
...
def main():
    ...
    for si,spec in enumerate(specs):
        ...
        for cond,box in crop_boxes(mask).items():
            x0,y0,x1,y1=box; rgbc=cv2.resize(rgb[y0:y1,x0:x1],(512,512),interpolation=cv2.INTER_AREA); K=effective_k(cams[0]['K'],box); pts=depth_points(depth,mask,table,box)
            if len(pts)>5000:
                rng=np.random.default_rng(int(hashlib.sha256(sid.encode()).hexdigest()[:16],16)); pts=pts[np.sort(rng.choice(len(pts),5000,replace=False))]
            bbox=np.array([[0,0,511,511]],np.float32); batch=recursive_to(prepare_batch(rgbc,est.transform,bbox,None,None),'cuda'); batch['cam_int']=torch.as_tensor(K[None],device='cuda').to(batch['img']); model._initialize_batch(batch)
            ...
            d={k:v.clone() for k,v in base.items()}; d['pred_cam_t'].requires_grad_(True); d['global_rot'].requires_grad_(True); d['body_pose'].requires_grad_(True); opt=torch.optim.Adam([{'params':[d['pred_cam_t']],'lr':0.01},{'params':[d['global_rot'],d['body_pose']],'lr':0.003}]); obs=torch.as_tensor(pts,device='cuda'); history=[]
            anchor_pick=torch.arange(0,len(fi),4,device='cuda')
            for it in range(O2_ITERS):
                opt.zero_grad(); vv=forward(d); anc=(vv[0][torch.as_tensor(faces[fi],device='cuda').long()]*torch.as_tensor(bc,device='cuda',dtype=vv.dtype)[:,:,None]).sum(1)[anchor_pick]; _,near_np=cKDTree(anc.detach().float().cpu().numpy()).query(obs.detach().float().cpu().numpy(),workers=-1); near=torch.as_tensor(near_np,device='cuda',dtype=torch.long); res=torch.linalg.norm(obs.float()-anc[near].float(),dim=1); cut=torch.quantile(res,.8); keep=res<=cut; loss=torch.nn.functional.smooth_l1_loss(res[keep],torch.zeros_like(res[keep]),beta=.01)+.01*(d['global_rot']-base['global_rot']).square().mean()+.01*(d['body_pose']-base['body_pose']).square().mean(); loss.backward();opt.step();history.append(float(loss.detach()))
            vp=forward(d).detach()[0].cpu().numpy()
            methods={'Official':vo,'Txyz':vt,'T+Pose':vp}; evals={}
            for method,v in methods.items():
                cams_eval={}
                for k in range(4):
                    vk=v if k==0 else transform(v,cams[0],cams[k]); pk=depth_points(cv2.imread(str(frame/f'k{k}.depth.png'),-1),cv2.imread(str(frame/f'k{k}.person_mask.jpg'),0),np.load(args.calibs/'intrinsics'/str(k)/'pointcloud_table.npy')); pk=deterministic_sample(pk,5000,sid+'sensor'+method+str(k)); ref_sample=deterministic_sample(ref_local[k],2000,sid+method+str(k)); cams_eval[f'K{k}']={'sensor':metric(pk,vk,faces,point_to_triangle_distances),'reference':metric(ref_sample,vk,faces,point_to_triangle_distances)}
                evals[method]=cams_eval
            row={'spec':spec,'condition':cond,'crop_box':list(box),'input_points':len(pts),'methods':evals,'txyz_m':txyz.tolist(),'txyz_trace':trace,'txyz_fallback':bool(np.linalg.norm(txyz)>TOTAL_TXYZ),'o2_final_loss':history[-1],'o2_initial_loss':history[0]}
            (args.out/'raw'/spec['subject']/spec['sequence']/spec['frame']).mkdir(parents=True,exist_ok=True); (args.out/'raw'/spec['subject']/spec['sequence']/spec['frame']/f'{cond}.json').write_text(json.dumps(row,indent=2)+'\n'); rows.append(row); print(si+1,len(specs),sid,cond,flush=True)
