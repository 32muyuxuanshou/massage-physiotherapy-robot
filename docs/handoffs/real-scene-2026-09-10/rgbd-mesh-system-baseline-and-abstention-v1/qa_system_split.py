...
ALLOWED_SPLITS = {"SYSTEM_VAL", "SYSTEM_SEALED"}
...
def assert_nonsealed_contract(plan: dict, workset_root: Path) -> None:
    if plan.get("selected_pixels") is not True or plan.get("final_reserve_pixels_selected") is not False:
        raise RuntimeError("plan does not explicitly exclude Final Deep Reserve pixels")
    sealed = set(plan["final_reserve_subjects_excluded"])
    rows = plan["observations"]
    subjects = {row["subject"] for row in rows}
    if any(row["split"] not in ALLOWED_SPLITS for row in rows) or subjects & sealed:
        raise RuntimeError("plan contains a non-frozen split or Final Deep Reserve subject")
    ...
    if sealed_present:
        raise RuntimeError(f"Final Deep Reserve directory present; refusing all pixel reads: {sorted(sealed_present)}")
...
def main() -> None:
    ...
    for name in ("winner-freeze", "plan", "extraction-report", "workset-root", "geometry-code-root", "output-root"):
    ...
    if plan.get("winner_checkpoint_sha256") != winner["checkpoint_sha256"]:
        raise RuntimeError("SEALED plan is not bound to the frozen winner")
    if not extraction.get("status", "").endswith("DECODE_PASS_GEOMETRY_QA_PENDING"):
        raise RuntimeError(f"selective extraction is not complete: {extraction.get('status')}")
    if extraction.get("final_reserve_pixels_materialized") is not False:
    ...
    report = {"status": "AUTO_QA_COMPLETE_VISUAL_REVIEW_PENDING", "task": "RGBD_MESH_SYSTEM_BASELINE_AND_ABSTENTION_V1",
              "scope": "SYSTEM_SPLIT_GEOMETRY_QA", "selected_split_pixels_read": True, "final_reserve_pixels_read": False,
              "source_plan": str(args.plan.resolve()), "source_plan_sha256": sha256(args.plan),
              "source_split_seed_from_plan": plan.get("subject_split_sha256"),
    ...
    write_json(args.output_root / "HUMMAN_SYSTEM_GEOMETRY_QA_V1.json", report)
