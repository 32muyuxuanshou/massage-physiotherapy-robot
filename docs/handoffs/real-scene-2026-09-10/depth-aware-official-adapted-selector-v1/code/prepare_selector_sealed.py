...
SEED = "humman-selector-sealed-20260910"
...
def plan(args):
    ...
    sealed=split["groups"]["SELECTOR_SEALED"]; reserve=set(split["groups"]["FINAL_RESERVE"])
    if set(sealed)&reserve or len(sealed)!=10: raise RuntimeError("invalid V2 SEALED identity split")
    ...
    for subject in sealed:
        ...
        observations.append({"subject":subject,"sequence":row["sequence"],"action":row["action"],
            "frame_ids":frames,"camera_candidates":list(CAMERAS),"split":"SELECTOR_SEALED"})
    depth_archive={}
    ...
    out={"status":"SELECTOR_SEALED_EXTRACTION_PLAN_FROZEN_AFTER_WINNER","winner":winner["winner"],
        ...
        "selector_sealed_subjects":sealed,"final_reserve_subjects_excluded":sorted(reserve),
...
def materialize(args):
    ...
    if p["status"]!="SELECTOR_SEALED_EXTRACTION_PLAN_FROZEN_AFTER_WINNER" or p["winner_checkpoint_sha256"]!=winner["checkpoint_sha256"]: raise RuntimeError("plan/winner mismatch")
    if p["final_reserve_pixels_selected"] is not False: raise RuntimeError("Final Reserve selection forbidden")
    if set(p["selector_sealed_subjects"])&set(p["final_reserve_subjects_excluded"]): raise RuntimeError("split overlap")
    ...
    for r in p["observations"]:
        for cam in CAMERAS:
            ...
            cap.release()
    report["status"]="SELECTOR_SEALED_DECODE_PASS_GEOMETRY_QA_PENDING"; write(args.report,report)
