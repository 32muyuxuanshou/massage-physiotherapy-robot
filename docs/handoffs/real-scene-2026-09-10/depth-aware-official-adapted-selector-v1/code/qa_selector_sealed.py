...
ALLOWED_SPLITS = {"SELECTOR_SEALED"}
...
def assert_nonsealed_contract(plan: dict, workset_root: Path) -> None:
    if plan.get("sealed_pixels_selected") is not True or plan.get("final_reserve_pixels_selected") is not False:
        raise RuntimeError("plan does not explicitly exclude SEALED pixels")
    sealed = set(set(plan["final_reserve_subjects_excluded"]))
...
def main() -> None:
    ...
    if extraction.get("status") != "SELECTOR_SEALED_DECODE_PASS_GEOMETRY_QA_PENDING":
    ...
    report = {"status": "AUTO_QA_COMPLETE_VISUAL_REVIEW_PENDING", "task": "PUBLIC_RGBD_SINGLE_VS_MULTIVIEW_V2",
              "scope": "SELECTOR_SEALED_OPENED_AFTER_WINNER_FREEZE", "sealed_pixels_read": True, "final_reserve_pixels_read": False,
    ...
    write_json(args.output_root / "HUMMAN_SELECTOR_SEALED_GEOMETRY_QA_V1.json", report)
