"""Plan or materialize V2 SEALED only after a VAL-selected winner is frozen."""
...
SEED = "humman-v2-sealed-eval-20260910"
...
def require_winner(path):
    w=read(path)
    if w.get("status") != "FROZEN_BEFORE_FIRST_V2_SEALED_ACCESS": raise RuntimeError("winner is not frozen before SEALED access")
...
def plan(args):
    ...
    sealed=split["groups"]["V2_SEALED"]; reserve=set(split["groups"]["FINAL_RESERVE"])
    if set(sealed)&reserve or len(sealed)!=12: raise RuntimeError("invalid V2 SEALED identity split")
    seqs=defaultdict(list)
    for r in inv["packed_coverage"]["sequence_records"]:
        if r["structurally_usable"] and r["subject"] in sealed: seqs[r["subject"]].append(r)
    observations=[]
    for subject in sealed:
        ...
        row=candidates[0]; cams={c["camera"]:c for c in row["cameras"]}; common=None
        for cam in CAMERAS:
        ...
        observations.append({"subject":subject,"sequence":row["sequence"],"action":row["action"],
            "frame_ids":frames,"camera_candidates":list(CAMERAS),"split":"V2_SEALED"})
    depth_archive={}
    archive_by_name={a["path"].replace("\\","/").rsplit("/",1)[-1]:a for a in inv["archives"]}
    for name,a in archive_by_name.items():
        if name.startswith("point_kinect_depth_part_"):
            for s in a["sequences"]: depth_archive[s]=name
    members=defaultdict(set)
    for r in observations:
        s=r["sequence"]; members["point_cameras.7z"].add(f"{s}/cameras.json")
        for cam in CAMERAS:
            members["point_kinect_color_part_02.7z"].add(f"{s}/kinect_color/{cam}.mp4")
            for frame in r["frame_ids"]:
                members[depth_archive[s]].add(f"{s}/kinect_depth/{cam}/{frame:06d}.png")
                members["point_kinect_mask.7z"].add(f"{s}/kinect_mask/{cam}/{frame:06d}.png")
    out={"status":"V2_SEALED_EXTRACTION_PLAN_FROZEN_AFTER_WINNER","winner":winner["winner"],
        "winner_checkpoint_sha256":winner["checkpoint_sha256"],"subject_split_sha256":sha(args.subject_split),
        "inventory_sha256":sha(args.inventory),"server_archive_root":str(args.archive_root),
        "server_workset_root":str(args.workset_root),"observations":observations,
        "v2_sealed_subjects":sealed,"final_reserve_subjects_excluded":sorted(reserve),
        "sealed_pixels_selected":True,"final_reserve_pixels_selected":False,
        "archives":[{"file":n,"sha256":archive_by_name[n]["prior_report_sha256_not_rehashed_this_scan"],"members":sorted(m)} for n,m in sorted(members.items())]}
    write(args.output,out)

def materialize(args):
    winner=require_winner(args.winner_freeze); p=read(args.plan)
    if p["status"]!="V2_SEALED_EXTRACTION_PLAN_FROZEN_AFTER_WINNER" or p["winner_checkpoint_sha256"]!=winner["checkpoint_sha256"]: raise RuntimeError("plan/winner mismatch")
    if p["final_reserve_pixels_selected"] is not False: raise RuntimeError("Final Reserve selection forbidden")
    if set(p["v2_sealed_subjects"])&set(p["final_reserve_subjects_excluded"]): raise RuntimeError("split overlap")
    ...
    for r in p["observations"]:
        for cam in CAMERAS:
            ...
            cap.release()
    report["status"]="V2_SEALED_DECODE_PASS_GEOMETRY_QA_PENDING"; write(args.report,report)

if __name__=="__main__":
    ap=argparse.ArgumentParser(); sub=ap.add_subparsers(dest="command",required=True)
    p=sub.add_parser("plan"); p.add_argument("--winner-freeze",type=Path,required=True); p.add_argument("--subject-split",type=Path,required=True); p.add_argument("--inventory",type=Path,required=True); p.add_argument("--archive-root",type=Path,required=True); p.add_argument("--workset-root",type=Path,required=True); p.add_argument("--output",type=Path,required=True)
