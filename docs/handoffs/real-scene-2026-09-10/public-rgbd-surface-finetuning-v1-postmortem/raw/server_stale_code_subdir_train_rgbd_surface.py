...
def evaluate(model, estimator, prepare_batch, recursive_to, rows: list, faces_np: np.ndarray,
    ...
    return {"aggregation": aggregate_subject_macro(details), "observations": details}


def save_checkpoint(path: Path, heads: dict, optimizer, epoch: int, counters: dict,
                    protocol_hash: str, workset_hash: str) -> dict:
...
def main() -> None:
    ...
    teacher = cache_official_teacher(model, estimator, prepare_batch, recursive_to, train_rows)
    ...
    baseline = evaluate(model, estimator, prepare_batch, recursive_to, val_rows, faces_np,
                        protocol["validation"], protocol["seed"])
    ...
    for epoch in range(1, run_cfg["epochs"] + 1):
        ...
        val = evaluate(model, estimator, prepare_batch, recursive_to, val_rows, faces_np,
                       protocol["validation"], protocol["seed"] + epoch)
        history.append({"epoch": epoch, "training_loss_component_means": {
            ...
            "validation": val, "counters": dict(counters), "elapsed_seconds": time.time() - start})
        ...
        checkpoints.append(save_checkpoint(checkpoint_path, heads, optimizer, epoch, counters,
                                           protocol_hash, workset_hash))

    expected_updates = run_cfg["epochs"] * math.ceil(len(train_rows) / accumulation)
    expected_exposures = run_cfg["epochs"] * len(train_rows)
    ...
    best_index = int(np.argmin([row["validation"]["aggregation"][primary_key] for row in history]))
    selection = None if args.mode == "smoke" else {
