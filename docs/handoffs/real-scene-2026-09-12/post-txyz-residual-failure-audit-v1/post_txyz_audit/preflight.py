...
from .io_v23 import load_json,sha256,snapshot_tree,assert_tree_unchanged
...
def run(v23_root,schema_path,manifest_path,back_region_path=None):
    before=snapshot_tree(v23_root);schema=load_json(schema_path)["features"]
    for feature in schema:validate_feature(feature)
    leakage=assert_no_leakage(schema);manifest=load_json(manifest_path);rows=manifest["rows"];ids={(r["subject"],r["sequence"],r["frame"]) for r in rows};source_path=Path(v23_root)/'report/BEHAVE_V2_FROZEN_TEST_MANIFEST.json';source=load_json(source_path);source_ids={(r['subject'],r['sequence'],r['frame']) for r in source['rows']};result_ids={(r['spec']['subject'],r['spec']['sequence'],r['spec']['frame']) for r in load_json(Path(v23_root)/'formal/report/per_frame_results.json')}
    checks={"rows_45":len(rows)==EXPECTED_FRAMES==len(ids),"subjects_5":{r['subject'] for r in rows}==EXPECTED_SUBJECTS,"sequences_15":len({r['sequence'] for r in rows})==EXPECTED_SEQUENCES,"all_cameras":all(tuple(r['cameras'])==('K0',*HELDOUT_CAMERAS) for r in rows),"known_failure_retained":KNOWN_ZERO_OF_THREE in ids,"v23_final_gate":load_json(Path(v23_root)/'formal/report/final_decision_v2.json')['gate']=='PASS_BEHAVE_CHEAP_TXYZ_GENERALIZATION_V2',"source_manifest_hash":manifest.get('source_sha256')==sha256(source_path),"exact_frame_identity":ids==source_ids==result_ids}
    back=load_frozen(back_region_path);assert_tree_unchanged(v23_root,before)
