...
def execute(v23_root,output_root,schema_path,manifest_path,sequence_root,replay_features_path,back_region_path=None,no_visuals=False):
    before=snapshot_tree(v23_root);schema=load_json(schema_path)['features'];assert_no_leakage(schema);rows=load_json(Path(v23_root)/'formal/report/per_frame_results.json')
    replay=load_json(replay_features_path);replay_by_id={item['frame_id']:item['features'] for item in replay['rows']};expected={"/".join((r['spec']['subject'],r['spec']['sequence'],r['spec']['frame'])) for r in rows};manifest_rows=load_json(manifest_path)['rows'];manifest_by_id={"/".join((r['subject'],r['sequence'],r['frame'])):r for r in manifest_rows}
