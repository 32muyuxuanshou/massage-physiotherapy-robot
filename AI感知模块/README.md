...
> 2026-09-07 最新进度：[用户已复核37点工程参考及数据资源决策](../docs/handoffs/real-scene-2026-09-06/reviewed-dmd37-2026-09-07/README.md)。真实8帧推理与mask对照已完成；37点真实投影尚未执行，真人精度和训练尚未验证。

本模块建立按摩理疗机器人的人体表面穴位先验与视觉定位链路。医生在冻结的 SKEL 皮肤上标注 Atlas，
已有内部工程使用 SKEL shape/pose 与虚拟相机验证 RGB-D 和关键点链路。2026-09-06 起主线转向真实
照片 mesh 拟合、Atlas 投影与校正生成训练标签，再训练预训练关键点模型。SMPL-X 是独立研究
工具，不与 SKEL 叠加。
...
2026-09-06 用户确认：真实场景工程精度优先，论文第二。已完成
[真实场景研究资料与实施设计](研究资料/真实场景关键点定位_2026-09-06/README.md)：30 篇索引、26 篇
本地 PDF、12 篇相关章节重点阅读；没有新增真实实验结果。

下一主任务仍为 `REAL_RGBD_DOMAIN_GATE_V1`，当前先执行真实照片 mesh/Atlas 产标子门：确认真实照片、
医生点定义和模型访问/算力，先运行 SAM mesh 拟合，验证跨模型 Atlas 对应、投影与标签质量，再用
原始照片＋筛选/校正标签训练 RTMPose。普通 RGB 即可开始二维训练，RGB-D 用于可选几何增强及后续
公制定位。SAM/NLF 优先作离线产标候选，在线是否使用单独验证；不直接替换正式 SKEL Atlas。
详见 [v2 重新评审](研究资料/真实场景关键点定位_2026-09-06/06_训练路线重新评审.md)。前版先人工逐图
标注训练、后加网格先验的顺序已被替代。当前已有 SAM 真实照片推理与 mask 对照；尚无独立真人定位精度验证，合成可靠性阈值不得直接继承。

网页端可直接阅读 [完整交接文档](../docs/real-scene-research-2026-09-06.md)。
