...
背部标注、预训练关键点模型和标定 RGB-D，人体网格先验作为待对照验证的候选。SMPL-X 是独立研究
工具，不与 SKEL 叠加。
...
下一主任务仍为 `REAL_RGBD_DOMAIN_GATE_V1`，具体执行真实数据路线：先冻结医生目标穴位及侧别，确认
真实 RGB-D 相机、标定与可用计算资源，建立按受试者隔离的自然俯卧背部数据；以公开预训练 RTMPose
真实微调＋深度测量作为首个基线，诊断现有合成模型域差距。SAM 3D Body/NLF 与 Atlas 的研究对应在
基线后独立验证，不直接替换正式 SKEL Atlas。合成数据保留作几何检查和辅助消融，当前 RTMPose-S
可靠性结果只冻结为合成工程候选，不能继承为真实域阈值。
