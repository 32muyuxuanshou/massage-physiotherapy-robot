#!/usr/bin/env python3
"""Probe shoulder Z compensation while keeping the rigid prone transform fixed."""
...
INDEX = {"head_twist": 25, "shoulder_r_x": 29, "shoulder_r_y": 30, "shoulder_r_z": 31, "elbow_flexion_r": 32, "shoulder_l_x": 39, "shoulder_l_y": 40, "shoulder_l_z": 41, "elbow_flexion_l": 42}
BASE = {"head_twist": 18.0, "shoulder_r_x": 80.0, "elbow_flexion_r": 25.0, "shoulder_l_x": -80.0, "elbow_flexion_l": 25.0}
CANDIDATES = {
    "ZP20": {**BASE, "shoulder_r_z": 20.0, "shoulder_l_z": -20.0},
    "ZM20": {**BASE, "shoulder_r_z": -20.0, "shoulder_l_z": 20.0},
    "ZP40": {**BASE, "shoulder_r_z": 40.0, "shoulder_l_z": -40.0},
    "ZM40": {**BASE, "shoulder_r_z": -40.0, "shoulder_l_z": 40.0},
}
...
def main() -> None:
    output = OUTPUT_ROOT / (datetime.now().strftime("%Y-%m-%d_%H-%M-%S") + "_depth_compensation")
    output.mkdir(parents=True, exist_ok=False)
    env = os.environ.copy()
    env.update({"BLENDER_USER_RESOURCES": str(WORKBENCH / "user_resources"), "ACUPOINT_MCP_PROJECT_ROOT": str(AI_ROOT), "PYTHONUTF8": "1", "PYTHONIOENCODING": "utf-8"})
    summary = {}
    for name, values in CANDIDATES.items():
        ...
        profile = {"schema": "skel-native-prone-depth-compensation-candidate-v1", "candidate": name, "medical_truth": False, "betas": [0.0] * 10, "pose_degrees": values, "pose_vector_degrees": [math.degrees(value) for value in pose]}
        ...
        run([str(BLENDER), "--background", str(CANONICAL), "--python", str(PREPARE), "--", "--snapshot", str(snapshot), "--result", str(prepare_result), "--native-pose-dir", str(native), "--pose-profile", str(profile_path), "--fixed-scene-contract", str(CONTRACT), "--width", "640", "--height", "512"], env=env, sentinel="ACU_PREPARE_PRONE_SCENE=PASS")
        run([str(BLENDER), "--background", str(snapshot), "--python", str(RENDER), "--", str(directory / "preview.png")], env=env, sentinel="ACU_NATURAL_PRONE_PREVIEW=PASS")
